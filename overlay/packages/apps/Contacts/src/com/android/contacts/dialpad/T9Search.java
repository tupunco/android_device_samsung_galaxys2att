...
class T9Search {
    ...
    private static boolean mIsChieseLocal = (Locale.getDefault().getLanguage().toLowerCase().equals(Locale.CHINESE.getLanguage().toLowerCase()));
    ...
    private void getAll() {
        ...
        while (contact.moveToNext()) {
            ...
            while (phone.getLong(1) == contactId) {
				...
				if (mIsChieseLocal) {
					String[] displayName = nameToNumber2(contact.getString(1));
					contactInfo.normalName = displayName[0];
					if (displayName.length > 1) {
						contactInfo.normalName2 = new ContactPinyinNormalName();
						contactInfo.normalName2.Number = displayName[1]; // search  number
						if (displayName.length > 2) {
							contactInfo.normalName2.Pinyin = displayName[2]; // pinyin cache
							contactInfo.normalName2.SpanCode = displayName[3];// span code
						}
					}
				} else
    ...
    public static class ContactPinyinNormalName{
    	String Number;
    	String Pinyin;
    	String SpanCode;
    }
    ...
    public static class ContactItem {
        ...
        ContactPinyinNormalName normalName2;
        int nameMatchLen;
    }
    ...
    public T9SearchResult search(String number) {
        ...
        for (ContactItem item : (newQuery ? mContacts : mAllResults)) {
            ...
            } else if (mIsChieseLocal && item.normalName2 != null && !TextUtils.isEmpty(item.normalName2.Number)
                    && !TextUtils.isEmpty(item.normalName2.SpanCode)) {
                pos = item.normalName2.Number.indexOf(number);
                if (pos != -1) {
                    int len = pos + number.length();
                    String spanCode = item.normalName2.SpanCode;
                    if (len <= spanCode.length() && (pos == 0 || spanCode.charAt(pos - 1) < spanCode.charAt(pos))) {
                        item.nameMatchId = Integer.valueOf(String.valueOf(spanCode.charAt(pos)));
                        item.nameMatchLen = ((len == spanCode.length()) ? Integer.valueOf(String.valueOf(spanCode
                                .charAt(len - 1))) + 1 : Integer.valueOf(String.valueOf(spanCode.charAt(len))))
                                - item.nameMatchId;
                        if (item.nameMatchLen <= 0)
                            item.nameMatchLen = 1;
                        mNameResults.add(item);
                    }
                }
     ...
     * 
     * @param name
     * @return [uppercase initial pinyin, all pinyin(each char's pinyin first
     *         char uppercase)]
     */
    private static String[] getNameLookupKeys(String name) {
        ...
        for (int i = tokenCount - 1; i >= 0; i--) {
            ...
            if (Token.PINYIN == token.type) {
                keyPinyin.insert(0, token.target.substring(0, 1) + token.target.substring(1).toLowerCase());
        ...
        return new String[] { keyInitial.toString(), keyPinyin.toString() };
    }

    /**
     * pinyin to SpanCode(seache result hightlight Span calc)
     * 
     * @param pinyin
     * @return
     */
    private static String pinyinToSpanCode(String pinyin, int hansLen) {
        if (TextUtils.isEmpty(pinyin) || hansLen >= 10)
            return null;
        ...
        char cChar = 'A';
        for (int i = 0; i < len; i++) {
            cChar = pinyin.charAt(i);
            if (cChar >= 'A' && cChar <= 'Z')
                cSpanIndex++;

            sb.append(cSpanIndex);
        }
        return sb.toString();
    }

    /**
     * Name to T9 number(only hans)
     * 
     * @param name
     * @return hans: [uppercase initial pinyin T9 number, all pinyin T9
     *         number(each char's pinyin first char uppercase), all pinyin (each
     *         char's pinyin first char uppercase), SpanCode]array other: [T9
     *         number] array
     */
    private static String[] nameToNumber2(String name) {
        if (TextUtils.isEmpty(name))
            return null;

        if (mIsChieseLocal) {
            final String[] names = getNameLookupKeys(name);
            if (names != null && names.length >= 2) {
                String[] onames = new String[4];

                onames[0] = nameToNumber(names[0]);
                onames[1] = nameToNumber(names[1]);
                onames[2] = names[1];
                onames[3] = pinyinToSpanCode(names[1], name.length());
                return onames;
            }
        }
        return new String[] { nameToNumber(name) };
    }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (o.name == null) {
                ...
                if (o.nameMatchId != -1) {
                    ...
                    int nameStart = 0;
                    if (o.normalName2 == null || o.nameMatchLen <= 0) {
                        nameStart = o.normalName.indexOf(mPrevInput);
                        s.setSpan(new ForegroundColorSpan(mContext.getResources().getColor(android.R.color.holo_blue_dark)), nameStart,
                                nameStart + mPrevInput.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
                    } else {
                        nameStart = o.nameMatchId;
                        s.setSpan(new ForegroundColorSpan(mContext.getResources().getColor(android.R.color.holo_blue_dark)), nameStart, 
                                nameStart + o.nameMatchLen, Spannable.SPAN_INCLUSIVE_INCLUSIVE);
                    }
